...
class LightServerWebsite {
    ...
    async init() {
        ...
        console.log('光鯖公式ホームページ初期化完了（アカウント同期対応版）');
    }
    ...
    async loadAllUsers() {
        try {
            ...
            if (snapshot.exists()) {
                ...
                console.log('全ユーザーリスト取得:', this.allUsers.length, '人');
            } else {
                this.allUsers = [];
            }
        } catch (error) {
    ...
    renderAccountPage() {
        ...
        // 管理者パスワードトリガー：user-roleにCtrl+Shift+左クリックイベント追加
        setTimeout(() => {
            const userRoleElement = document.querySelector('.user-role');
            if (userRoleElement) {
                userRoleElement.addEventListener('click', (e) => {
                    if (e.ctrlKey && e.shiftKey) {
                        ...
                        if (password === 'atsuki0622') {
                            this.updateUserRoleInFirebase(this.currentUser.id, 'admin');
    ...
    async changeUserRole(userId, newRole) {
        ...
        try {
            ...
            if (success) {
                alert('権限を更新しました');
                await this.renderPermissionsTab(); // 一覧を再描画
            } else {
                alert('権限の更新に失敗しました');
            }
        } catch (error) {
